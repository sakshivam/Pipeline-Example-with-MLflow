...
def Columns_to_drop(df):
    # Find out which columns have null values and total number of
    # Nan values in that column
    # Determine percentage of missing data in a particular column
...
def Missing_Val_Columns_to_fill(df):
    # Finding the value counts for columns with percent_missing between 0 and
    #  95%
    # Replacing Nan values in each of these columns with most frequent value in
    #  that column.
    percent_missing = df.isnull().sum() * 100 / len(df)
