...
import yaml
from src.utils.scoring_utils import Fill_missing_Val_Columns, Drop_Missing_Val_Columns, replace_values_using_dict
...
testdf = Fill_missing_Val_Columns(testdf, config['MISSING_VAL_COLUMNS_TO_FILL'])
...
testdf=replace_values_using_dict(testdf,config['dict_to_get_ordinal_features'] )
model_file_path = os.path.join(data_dir, "step5\\model.joblib")
result_df = predictions(testdf, 'Y', config['TEN_BEST_FEATURES_OBSERVED_SELECTION'], model_file_path)
